    ...
    >
      <div className="h-10 w-full bg-gray-900 flex items-center justify-between px-[300px]">
        <span className=" text-white">
          Authorised Channel Partner of Milton Roy
        </span>
        <span className="text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center justify-center space-x-6">
            ...
            >
              <span className="text-black hover:text-gray-600 transition-colors cursor-pointer">
                Products
              </span>
              <span className="absolute bottom-0 left-0 w-full h-0.5 bg-black transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left"></span>
              ...
              {showProductMenu && (
                <div className="absolute left-1/2 transform -translate-x-1/2 mt-2 bg-white shadow-lg rounded-xl py-2 min-w-[300px] border border-gray-200 transition-all duration-300">
                  {Object.values(products).map((category) => (
                    ...
                    >
                      <div className="px-4 py-2 hover:bg-gray-100 rounded-lg cursor-pointer transition-all duration-300 flex items-center">
                        <span className="text-sm font-medium group-hover/item:text-gray-700 transition-colors">
                          {category.name}
                        </span>
                        <span className="ml-auto text-gray-400 group-hover/item:text-gray-600 transition-colors transform group-hover:item:translate-x-1 ">
                        <ArrowRight />
                        </span>
                      </div>

                      {activeCategory === category.name && (
                        <div className="absolute left-full top-0 ml-2 bg-white shadow-lg rounded-xl py-2 min-w-[280px] border border-gray-200 transition-transform duration-300">
                          {category.items.map((item) => (
                            <div key={item} className="relative group/sub">
                              <Link
                                href={`/product/${category.name.toLowerCase()}/${item.toLowerCase()}`}
                                className="block px-4 py-2 text-sm hover:bg-gray-100 rounded-lg transition-all duration-300"
                              >
                                <span className="relative group-hover/sub:pl-3 transition-all duration-300 inline-block">
                                  {item}
                                </span>
                              </Link>
                            </div>
